...
def main() -> None:
    ...
    assert ".metric-detail-section" in css_contract, css_contract[:9000]
    assert ".detail-section-kicker" in css_contract, css_contract[:9000]
    assert ".section-head {\n      display: grid;\n      grid-template-columns: minmax(0, 1fr);" in css_contract, css_contract[:7000]
    assert "grid-template-columns: minmax(0, 246px) minmax(0, 1fr)" not in css_contract, css_contract[:7000]
    assert ".metrics-primary {\n      display: grid;\n      grid-template-columns: minmax(360px, 0.92fr) minmax(0, 1.08fr);" in css_contract, css_contract[:9000]
    assert ".metric-detail-grid {\n      grid-template-columns: repeat(2, minmax(min(100%, 420px), 1fr));" in css_contract, css_contract[:9000]
    assert ".metric-card {\n      display: grid;\n      grid-template-columns: minmax(112px, 0.28fr) minmax(0, 1fr);" in css_contract, css_contract[:11000]
    assert "repeat(auto-fit, minmax(min(100%, 320px), 1fr))" not in css_contract, css_contract[:9000]
    ...
    assert 'aria-label="指标图表与成熟度摘要"' in report_html, report_html[:9000]
    assert 'class="metrics-stack"' not in report_html, report_html[:9000]
    assert 'class="metric-detail-section"' in report_html, report_html[:12000]
    assert 'class="detail-section-kicker"' in report_html, report_html[:12000]
    assert "指标明细" in report_html, report_html[:12000]
